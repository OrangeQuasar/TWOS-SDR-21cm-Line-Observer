...
```
### [こちら](https://zadig.akeo.ie/) から最新版をダウンロード・実行してSDR用のドライバーを設定
...
```
### 必要情報の入力
- Source (天体名)、Duration (積分時間 [秒])を入力　※No (観測番号)はデフォルトで管理されます。特定の番号を入力することも可能です。
...
### 解析モードについて
「解析モード」 は、保存済みのCSVファイル（観測データ）を2つ読み込み、それらの差分を計算してグラフ表示・ファイル保存する機能です。以下のような用途などで使用できます。
- 観測後にデータを再確認したい場合。

- dB計算ではなく、単純な強度の引き算（リニア差分）で信号を見たい場合。

- 異なるタイミングで撮った「空（OFF）」のデータを基準にして再解析したい場合。
